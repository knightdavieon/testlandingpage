...
function loadGeofences() {
    ...
    .then(data => {
        ...
        data.forEach(function(geofence) {
            ...
            geofenceLayers.push({ id: geofence.id, name: geofence.name, layer: geoJsonLayer });
        });
    })
...
function checkGeofence(lat, lng) {
    ...
    drawnItems.eachLayer(function(layer) {
        ...
        if (turf.booleanPointInPolygon(point, poly)) {
            ...
            if (geofence) {
                geofencesInside.push({ id: geofence.id, name: geofence.name });
            }
        }
    });
    ...
    if (insideAnyGeofence && !lastInsideGeofence) {
        let message = 'You have entered the following geofence(s):\n' + 
                      geofencesInside.map(g => `ID: ${g.id}, Name: ${g.name}`).join('\n');
        Swal.fire({
